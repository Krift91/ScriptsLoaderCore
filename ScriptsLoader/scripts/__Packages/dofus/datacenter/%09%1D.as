class dofus.datacenter.§\t\x1d§ extends ank.battlefield.datacenter.§\x1e\x0e\x10§
{
   var api;
   var _nBonusValue;
   var _aNamesList;
   var _aLevelsList;
   var _sDefaultAnimation = "static";
   var _bAllDirections = false;
   var _bForceWalk = true;
   var _nAlignmentIndex = -1;
   function §\t\x1d§(sID, clipClass, §\x1e\x11\x1c§, §\x13\x05§, §\x10\x1d§, bonus)
   {
      super();
      this.api = _global.API;
      this._nBonusValue = bonus;
      this.initialize(sID,clipClass,_loc5_,_loc6_,_loc7_,null);
   }
   function set name(§\x1e\n\x02§)
   {
      this._aNamesList = new Array();
      var _loc3_ = _loc2_.split(",");
      var _loc4_ = 0;
      while(_loc4_ < _loc3_.length)
      {
         var _loc5_ = this.api.lang.getMonstersText(_loc3_[_loc4_]);
         this._aNamesList.push(_loc5_.n);
         if(_loc5_.a != -1)
         {
            this._nAlignmentIndex = _loc5_.a;
         }
         _loc4_ = _loc4_ + 1;
      }
   }
   function get name()
   {
      return this.getName();
   }
   function getName(§\x1e\x12\x16§)
   {
      _loc2_ = _loc2_ != undefined ? _loc2_ : "\n";
      var _loc3_ = new Array();
      var _loc4_ = 0;
      while(_loc4_ < this._aLevelsList.length)
      {
         _loc3_.push({level:Number(this._aLevelsList[_loc4_]),name:this._aNamesList[_loc4_]});
         _loc4_ = _loc4_ + 1;
      }
      _loc3_.sortOn(["level"],Array.DESCENDING | Array.NUMERIC);
      var _loc5_ = new String();
      var _loc6_ = 0;
      while(_loc6_ < _loc3_.length)
      {
         var _loc7_ = _loc3_[_loc6_];
         _loc5_ += _loc7_.name + " (" + _loc7_.level + ")" + _loc2_;
         _loc6_ = _loc6_ + 1;
      }
      return _loc5_;
   }
   function alertChatText()
   {
      var _loc2_ = this.api.datacenter.Map;
      return "Groupe niveau " + this.totalLevel + " en " + _loc2_.x + "," + _loc2_.y + " : <br/>" + this.getName("<br/>");
   }
   function set Level(§\x1e\n\x02§)
   {
      this._aLevelsList = _loc2_.split(",");
   }
   function get totalLevel()
   {
      var _loc2_ = 0;
      var _loc3_ = 0;
      while(_loc3_ < this._aLevelsList.length)
      {
         _loc2_ += Number(this._aLevelsList[_loc3_]);
         _loc3_ = _loc3_ + 1;
      }
      return _loc2_;
   }
   function get bonusValue()
   {
      return this._nBonusValue;
   }
   function get alignment()
   {
      return new dofus.datacenter["\x1e\f"](this._nAlignmentIndex,0);
   }
}
