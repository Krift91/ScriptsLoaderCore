class dofus.§\x0b\b§.SpeakingItemsManager extends dofus.§\x1e\n\x07§.§\x1d\x19§
{
   ...
   {
      super();
      dofus["\x0b\b"].SpeakingItemsManager._sSelf = this;
      this.initialize(oAPI);
   }
   static function getInstance()
   {
      return dofus["\x0b\b"].SpeakingItemsManager._sSelf;
   }
   ...
   function triggerPrivateEvent(§\x1e\x12\x0e§)
   {
      this.api.kernel.AudioManager.playSound(_loc2_);
   }
   function triggerEvent(§\x05\x12§)
   {
      if(_loc2_ == dofus["\x0b\b"].SpeakingItemsManager.SPEAK_TRIGGER_ON_CONNECT)
      {
         ank["\x1e\n\x07"]["\x1e\x0b\x02"].removeTimer(this,"SpeakingItemsManager",dofus["\x0b\b"].SpeakingItemsManager._nTimer);
         ank["\x1e\n\x07"]["\x1e\x0b\x02"].setTimer(this,"SpeakingItemsManager",this,this.triggerEvent,dofus["\x0b\b"].SpeakingItemsManager.MINUTE_DELAY,[dofus["\x0b\b"].SpeakingItemsManager.SPEAK_TRIGGER_MINUTE],true);
      }
      ...
      if(this._eaItems.length)
      {
         ...
         {
            var _loc4_ = this.api.lang.getSpeakingItemsTrigger(_loc2_)[_loc3_.mood];
            if(_loc4_)
            {
   ...
   function generateNextMsgCount(§\x17\x05§)
   {
