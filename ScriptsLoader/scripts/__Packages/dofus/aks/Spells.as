...
{
   function Spells(§\x1e\x1a\x0e§, oAPI)
   {
      super.initialize(_loc3_,oAPI);
   }
   function moveToUsed(§\x04\x12§, §\x01\n§)
   {
      this.aks.send("SM" + _loc2_ + "|" + _loc3_,false);
   }
   function boost(§\x04\x12§)
   {
      this.aks.send("SB" + _loc2_);
   }
   function spellForget(§\x04\x12§)
   {
      this.aks.send("SF" + _loc2_);
   }
   function onUpgradeSpell(§\x14\x16§, §\x1e\x12\r§)
   {
      if(_loc2_)
      {
         var _loc4_ = this.api.kernel.CharactersManager.getSpellObjectFromData(_loc3_);
   ...
   function onChangeOption(§\x1e\x12\r§)
   {
      this.api.datacenter.Basics.canUseSeeAllSpell = _loc2_.charAt(0) == "+";
   }
   function onSpellBoost(§\x1e\x12\r§)
   {
      var _loc3_ = _loc2_.split(";");
   ...
   function onSpellForget(§\x1e\x12\r§)
   {
      if(_loc2_ == "+")
      {
