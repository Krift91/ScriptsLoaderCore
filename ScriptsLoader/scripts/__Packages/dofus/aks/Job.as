...
{
   function Job(§\x1e\x1a\x0e§, oAPI)
   {
      super.initialize(_loc3_,oAPI);
   }
   function changeJobStats(§\x04\x02§, §\x1e\x17\x02§, minSlots)
   {
      this.aks.send("JO" + _loc2_ + "|" + _loc3_ + "|" + minSlots);
   }
   function onSkills(§\x1e\x12\r§)
   {
      ...
      while(_loc5_ < _loc3_.length)
      {
         ...
         while(_loc10_-- > 0)
         {
            ...
            _loc8_.push(new dofus.datacenter["\x1e\x10\x1d"](_loc11_[0],_loc11_[1],_loc11_[2],_loc11_[3],_loc11_[4]));
         }
   ...
   function onOptions(§\x1e\x12\r§)
   {
      var _loc3_ = _loc2_.split("|");
      var _loc4_ = Number(_loc3_[0]);
      var _loc5_ = Number(_loc3_[1]);
      var _loc6_ = Number(_loc3_[2]);
      this.api.datacenter.Player.Jobs[_loc4_].options = new dofus.datacenter["\f\x07"](_loc5_,_loc6_);
   }
}
