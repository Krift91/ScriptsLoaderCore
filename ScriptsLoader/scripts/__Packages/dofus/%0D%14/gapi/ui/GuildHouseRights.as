class dofus.§\r\x14§.gapi.ui.GuildHouseRights extends dofus.§\r\x14§.gapi.core.§\x10\x19§
{
   var _hHouse;
   var _winBackground;
   ...
   function set house(§\r\x06§)
   {
      this._hHouse = _loc2_;
   }
   function init()
   {
      super.init(false,dofus["\r\x14"].gapi.ui.GuildHouseRights.CLASS_NAME);
   }
   ...
   function update()
   {
      ...
      this._btnShowEmblemForGuild.selected = this._hHouse.isShared && this._hHouse.hasRight(dofus.datacenter["\r\x04"].GUILDSHARE_DOORSIGN_GUILD);
      this._btnShowEmblemForOthers.selected = this._hHouse.isShared && this._hHouse.hasRight(dofus.datacenter["\r\x04"].GUILDSHARE_DOORSIGN_OTHERS);
      this._btnAllowGuildToAccessHouse.selected = this._hHouse.isShared && this._hHouse.hasRight(dofus.datacenter["\r\x04"].GUILDSHARE_ALLOWDOOR_GUILD);
      this._btnDenyOtherToAccessHouse.selected = this._hHouse.isShared && this._hHouse.hasRight(dofus.datacenter["\r\x04"].GUILDSHARE_FORBIDDOOR_OTHERS);
      this._btnAllowGuildToAccessSafes.selected = this._hHouse.isShared && this._hHouse.hasRight(dofus.datacenter["\r\x04"].GUILDSHARE_ALLOWCHESTS_GUILD);
      this._btnDenyOtherToAccessSafes.selected = this._hHouse.isShared && this._hHouse.hasRight(dofus.datacenter["\r\x04"].GUILDSHARE_FORBIDCHESTS_OTHERS);
      this._btnAllowRespawn.selected = this._hHouse.isShared && this._hHouse.hasRight(dofus.datacenter["\r\x04"].GUILDSHARE_RESPAWN);
      this._btnAllowTeleport.selected = this._hHouse.isShared && this._hHouse.hasRight(dofus.datacenter["\r\x04"].GUILDSHARE_TELEPORT);
   ...
   function validate()
   {
      ...
      {
         _loc2_ += dofus.datacenter["\r\x04"].GUILDSHARE_DOORSIGN_GUILD;
      }
      if(this._btnShowEmblemForOthers.selected)
      {
         _loc2_ += dofus.datacenter["\r\x04"].GUILDSHARE_DOORSIGN_OTHERS;
      }
      if(this._btnAllowGuildToAccessHouse.selected)
      {
         _loc2_ += dofus.datacenter["\r\x04"].GUILDSHARE_ALLOWDOOR_GUILD;
      }
      if(this._btnDenyOtherToAccessHouse.selected)
      {
         _loc2_ += dofus.datacenter["\r\x04"].GUILDSHARE_FORBIDDOOR_OTHERS;
      }
      if(this._btnAllowGuildToAccessSafes.selected)
      {
         _loc2_ += dofus.datacenter["\r\x04"].GUILDSHARE_ALLOWCHESTS_GUILD;
      }
      if(this._btnDenyOtherToAccessSafes.selected)
      {
         _loc2_ += dofus.datacenter["\r\x04"].GUILDSHARE_FORBIDCHESTS_OTHERS;
      }
      if(this._btnAllowRespawn.selected)
      {
         _loc2_ += dofus.datacenter["\r\x04"].GUILDSHARE_RESPAWN;
      }
      if(this._btnAllowTeleport.selected)
      {
         _loc2_ += dofus.datacenter["\r\x04"].GUILDSHARE_TELEPORT;
      }
      this.api.network.Houses.rights(_loc2_);
      this.unloadThis();
   }
   function guild(§\x1e\x19\x0e§)
   {
      this.update();
   }
   function shared(§\x1e\x19\x0e§)
   {
      this.update();
   }
