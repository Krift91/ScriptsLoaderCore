class dofus.§\r\x14§.gapi.ui.Login extends dofus.§\r\x14§.gapi.core.§\x10\x19§
{
   var _mcGoToStatus;
   var _lblGoToStatus;
   ...
   {
      this._sLanguage = _loc2_;
   }
   function set canAutoLogOn(§\x1b\x17§)
   {
      this._bCanAutoLogOn = _loc2_;
   }
   function isLoaded()
   {
   ...
   function autoLogin(§\x1e\x10\x0b§, §\x1e\x0f\x04§)
   {
   ...
   function zaapAutoLogin(§\x19\x1b§)
   {
      if(!this.usingZaapConnect())
      {
         return undefined;
      }
      if(!_loc2_ && getTimer() - this._nLastRegisterTime < 1000)
      {
         return undefined;
      }
      this._nLastRegisterTime = getTimer();
      var _loc3_ = dofus["\x1e\t\x06"].getInstance().consumeAuthToken();
      if(_loc3_ == undefined)
      {
         dofus["\x1e\t\x06"].getInstance().renewAuthKey();
         return undefined;
      }
      var _loc4_ = dofus["\x1e\t\x06"].LOGIN_TOKEN_NAME;
      this.onLogin(_loc4_,_loc3_,true);
   }
   function refreshAutoLoginUi()
   {
      var _loc2_ = this.usingZaapConnect();
      this._btnOK._visible = !_loc2_;
      this._btnForget._visible = !_loc2_;
      this._tiAccount._visible = !_loc2_;
      this._tiPassword._visible = !_loc2_;
      this._lblAccount._visible = !_loc2_;
      this._lblPassword._visible = !_loc2_;
      this._lblForget._visible = !_loc2_;
      this._lblSubscribe._visible = !_loc2_;
      this._mcSubscribe._visible = !_loc2_;
      this._mcPasswordIdentification._visible = !_loc2_;
      this._lblAutoconnect._visible = _loc2_;
      this._mcAutoconnect._visible = _loc2_;
      this._mcCaution._visible = !_global.CONFIG.isStreaming && !_loc2_;
   }
   function usingZaapConnect()
   {
      if(!dofus["\x1e\t\x06"].ENABLED)
      {
         return false;
      }
      var _loc2_ = dofus["\x1e\t\x06"].getInstance();
      return _loc2_ != null && _loc2_.getSessionToken() != null;
   }
   function init()
   {
      super.init(false,dofus["\r\x14"].gapi.ui.Login.CLASS_NAME);
   }
   function createChildren()
   {
      ...
      {
         var _loc2_ = this.api.lang.getConfigText("ALERTY_LINK");
         if(_loc2_ != "")
         {
            this._xAlert = new XML();
            this._xAlert.ignoreWhite = true;
            var _owner = this;
            this._xAlert.onLoad = function(§\x14\x16§)
            {
               _owner.onAlertLoad(_loc2_);
            };
            this._xAlert.load(_loc2_);
         }
      }
   ...
   function loadGifts()
   {
      ...
      {
         this.owner.onGifts(this,_loc2_);
      };
   ...
   function fillCommunityID()
   {
      ...
      else
      {
         ...
         if(_loc5_[0] == "??" || (_loc5_[1] == "?" || (_loc5_ == undefined || (_loc5_[0] == undefined || (_loc5_[1] == undefined || _global.isNaN(_loc5_[1]))))))
         {
            ...
            this._lvCountry.onLoad = function(§\x14\x16§)
            {
               ref.onCountryLoad(_loc2_);
            };
   ...
   function showLastAlertButton(§\x15\x0e§)
   {
      if(this.usingZaapConnect() && _loc2_)
      {
         return undefined;
      }
      this._btnShowLastAlert._visible = _loc2_;
      this._mcCaution._visible = _loc2_;
   }
   function switchLanguage(§\x1e\x10\x16§)
   {
   ...
   function getCommunityFromCountry(§\x1e\x13\x0f§)
   {
   ...
   function itemSelected(§\x1e\x19\x0e§)
   {
      switch(_loc2_.target._name)
      {
         case "_cbPorts":
            ...
            break;
         case "_lstNews":
            var _loc4_ = ank["\x1e\n\x07"].rss["\x1e\x15\x03"](_loc2_.row.item);
            this.getURL(_loc4_.getLink(),"_blank");
      }
   }
   function onLogin(§\x1e\x10\x0b§, §\x1e\x0f\x03§, §\x13\x11§)
   {
      ...
      if(!_loc4_)
      {
         if(dofus.Constants.DEBUG)
         {
            ...
            _loc5_.data.loginInfos = {account:_loc2_,password:_loc3_};
            _loc5_.close();
         }
         else if(this.api.kernel.OptionsManager.getOption("RememberAccountName"))
         {
            this.api.kernel.OptionsManager.setOption("LastAccountNameUsed",_loc2_);
         }
      }
      this.api.datacenter.Player.login = _loc2_;
      if(_loc4_)
      {
         this.api.datacenter.Player.zaapToken = _loc3_;
         this.api.datacenter.Player.password = undefined;
      }
      else
      {
         this.api.datacenter.Player.password = _loc3_;
      ...
      {
         var _loc6_ = this.api.lang.getConfigText("SERVER_NAME");
         var _loc7_ = new ank["\x1e\n\x07"]["\x0f\x01"]();
         var _loc8_ = Math.floor(Math.random() * _loc6_.length);
         var _loc9_ = 0;
      ...
      {
         var _loc10_ = this.api.lang.getText("NO_SERVER_ADDRESS");
         this.api.kernel.showMessage(this.api.lang.getText("CONNECTION"),_loc10_ != undefined ? _loc10_ : "Erreur interne\nContacte l\'équipe Dofus","ERROR_BOX",{name:"OnLogin"});
      }
      else
      {
   ...
   {
      this._bHideNext = _loc2_.hideNext;
      var _loc3_ = SharedObject.getLocal(dofus.Constants.OPTIONS_SHAREDOBJECT_NAME);
      _loc3_.data.lastAlertID = !_loc2_.hideNext ? undefined : this._sAlertID;
      _loc3_.flush();
      ...
      switch(_loc2_.target._name)
      {
         ...
         case "_mcSubscribe":
            ...
            if(this.api.lang.getConfigText("REGISTER_INGAME"))
            {
               ...
               var _loc4_ = dofus["\r\x14"].gapi.ui.Register(_loc3_);
               _loc4_.addEventListener("close",this);
            }
         ...
         case "_mcGoToStatus":
            ...
            break;
         case "_btnRememberMe":
            this.api.kernel.OptionsManager.setOption("RememberAccountName",_loc2_.target.selected);
            break;
         default:
   ...
   function onRSSLoadError(§\x1e\x19\x0e§)
   {
      ank["\x1e\n\x07"]["\x0b\f"].err("Impossible de charger le fichier RSS");
   }
   function onBadRSSFile(§\x1e\x19\x0e§)
   {
      ank["\x1e\n\x07"]["\x0b\f"].err("Fichier RSS invalide");
   }
   function onRSSLoaded(§\x1e\x19\x0e§)
   {
      var _loc3_ = ank["\x1e\n\x07"].rss["\x1e\x15\x02"](_loc2_.target);
      var _loc4_ = new ank["\x1e\n\x07"]["\x0f\x01"]();
      _loc4_.createFromArray(_loc3_.getChannels()[0].getItems());
      this._lstNews.dataProvider = _loc4_;
   }
   function onGifts(§\x1e\x18\x18§, §\x14\x16§)
   {
      var _loc4_ = 0;
      if(_loc3_ && !_global.CONFIG.isStreaming)
      {
         ...
         while(_loc6_ <= _loc4_)
         {
            ...
            this._aGiftsURLs.push({id:_loc6_,url:_loc2_["g" + _loc6_]});
            ...
            _loc8_.contentPath = dofus.Constants.GIFTS_PATH + _loc2_["g" + _loc6_];
            _loc6_ = _loc6_ + 1;
         }
         if(_loc4_ > 5)
         {
   ...
   function over(§\x1e\x19\x0e§)
   {
      ...
      {
         case "_mcPurple":
            this.gapi.showTooltip(this.api.lang.getText("PURPLE_DOFUS"),_loc2_.target,-50);
            break;
         case "_mcEmerald":
            this.gapi.showTooltip(this.api.lang.getText("EMERALD_DOFUS"),_loc2_.target,-50);
            break;
         case "_mcTurquoise":
            this.gapi.showTooltip(this.api.lang.getText("TURQUOISE_DOFUS"),_loc2_.target,-50);
            break;
         case "_mcEbony":
            this.gapi.showTooltip(this.api.lang.getText("EBONY_DOFUS"),_loc2_.target,-50);
            break;
         case "_mcIvory":
            this.gapi.showTooltip(this.api.lang.getText("IVORY_DOFUS"),_loc2_.target,-50);
            break;
         case "_mcOchre":
            this.gapi.showTooltip(this.api.lang.getText("OCHRE_DOFUS"),_loc2_.target,-50);
            break;
         default:
            if(String(_loc2_.target._name).substring(0,7) == "_mcFlag")
            {
            ...
            this.gapi.showTooltip(_loc2_.target.params.description,_loc2_.target,-40);
            break;
      }
   }
   ...
   function onEvolutionsPostCount(§\x1e\x18\x18§)
   {
      var _loc3_ = SharedObject.getLocal(dofus.Constants.OPTIONS_SHAREDOBJECT_NAME);
      this._nForumEvolutionsPostCount = Number(_loc2_.c);
   ...
   function onServersStatePostCount(§\x1e\x18\x18§)
   {
      var _loc3_ = SharedObject.getLocal(dofus.Constants.OPTIONS_SHAREDOBJECT_NAME);
      this._nForumServersStatePostCount = Number(_loc2_.c);
   ...
   function onCountryLoad(§\x14\x16§)
   {
