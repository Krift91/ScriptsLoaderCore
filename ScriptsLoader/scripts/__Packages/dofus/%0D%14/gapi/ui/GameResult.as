class dofus.§\r\x14§.gapi.ui.GameResult extends dofus.§\r\x14§.gapi.core.§\x10\x19§
{
   var _oData;
   var _lblBonus;
   var _sdStars;
   ...
   function init()
   {
      super.init(false,dofus["\r\x14"].gapi.ui.GameResult.CLASS_NAME);
   ...
   function initData()
   {
      ...
      var _loc6_ = Math.min(_loc2_,dofus["\r\x14"].gapi.ui.GameResult.MAX_VISIBLE_PLAYERS_IN_TEAM) * 20 + 65 + Math.min(_loc3_,dofus["\r\x14"].gapi.ui.GameResult.MAX_VISIBLE_PLAYERS_IN_TEAM) * 20 + 65;
      if(_loc4_ > 0)
      {
         _loc6_ += Math.min(_loc4_,dofus["\r\x14"].gapi.ui.GameResult.MAX_VISIBLE_PLAYERS_IN_TEAM) * 20 + 65;
      }
      var _loc7_ = _loc6_ + 32;
      var _loc8_ = ((_loc5_ <= dofus["\r\x14"].gapi.ui.GameResult.MAX_PLAYERS ? this.gapi.screenHeight : 550) - _loc7_) / 2;
      var _loc9_ = this._winBackground._x + 10;
      var _loc10_ = _loc8_ + 32;
      var _loc11_ = Math.min(_loc2_,dofus["\r\x14"].gapi.ui.GameResult.MAX_VISIBLE_PLAYERS_IN_TEAM) * 20 + 55 + _loc10_;
      var _loc12_ = Math.min(_loc3_,dofus["\r\x14"].gapi.ui.GameResult.MAX_VISIBLE_PLAYERS_IN_TEAM) * 20 + 55 + _loc11_;
      ...
      if(this._oData.challenges && this._oData.challenges.length)
      {
         ...
         {
            var _loc14_ = dofus["\r\x14"].gapi.controls.FightChallengeIcon(this.attachMovie("FightChallengeIcon","fci" + _loc15_,this.getNextHighestDepth(),{challenge:this._oData.challenges[_loc15_],displayUiOnClick:false}));
            var _loc0_ = null;
   ...
   {
      var _loc0_ = null;
      if((_loc0_ = _loc2_.target) === this._sdStars)
      {
