class dofus.§\r\x14§.gapi.ui.MapExplorer extends dofus.§\r\x14§.gapi.core.§\x10\x19§
{
   var _dmMap;
   var _sPointer;
   var _mnMap;
   ...
   function set mapID(§\x03\x0b§)
   {
      this._dmMap = new dofus.datacenter["\x10\x15"](_loc2_);
   }
   function set pointer(§\x1e\x0e\x13§)
   {
      this._sPointer = _loc2_;
   }
            ...
            if(_loc9_ == this.dungeonID || _global.isNaN(this.dungeonID))
            {
               ...
               switch(_loc4_)
               {
                  ...
                  case 2:
                     ...
                     _loc8_ = _loc5_ + "," + _loc6_ + " (" + _global.API.ui.getUIComponent("Party").getMemberById(_loc2_[k].playerID).name + ")";
                     if(_loc8_ != undefined)
                     {
                        break;
                     }
   ...
   function init()
   {
      super.init(false,dofus["\r\x14"].gapi.ui.MapExplorer.CLASS_NAME);
   ...
   function showMapSuperArea(§\x1e\x1c\x12§)
   {
      if(_loc2_ == undefined)
      {
         return undefined;
      }
      this._mnMap.contentPath = dofus.Constants.LOCAL_MAPS_PATH + _loc2_ + ".swf";
      this._mnMap.clear();
   ...
   function hideArrows(§\x19\t§)
   {
      this._mcTriangleNW._visible = this._mcTriangleN._visible = this._mcTriangleNE._visible = this._mcTriangleW._visible = this._mcTriangleE._visible = this._mcTriangleSW._visible = this._mcTriangleS._visible = this._mcTriangleSE._visible = !_loc2_;
   }
   function showHintsCategory(categoryID, §\x15\x0e§)
   {
      ...
      this["_mcFilter" + categoryID].selected = _loc3_;
   ...
   function drawHintsOnCategoryLayer(categoryID)
   {
      ...
      if(dofus["\r\x14"].gapi.ui.MapExplorer.FILTER_CONQUEST_ID == categoryID)
      {
      ...
      {
         var _loc6_ = new dofus.datacenter["\r\x05"](_loc4_[_loc5_]);
         if((_loc6_.superAreaID == this._dmMap.superarea || dofus["\r\x14"].gapi.ui.MapExplorer.FILTER_CONQUEST_ID == categoryID && categoryID != 5) && _loc6_.y != undefined)
         {
   ...
   function getConquestAreaList()
   {
      ...
      {
         this.addToQueue({object:this,method:this.drawHintsOnCategoryLayer,params:[dofus["\r\x14"].gapi.ui.MapExplorer.FILTER_CONQUEST_ID]});
      }
   ...
   function initDungeonMap(§\x05\x1a§)
   {
      var _loc3_ = this.api.datacenter.Map;
      this._mnMap.clear();
   ...
   function over(§\x1e\x19\x0e§)
   {
      switch(_loc2_.target)
      {
         case this._mnMap:
            ...
            this.setMovieClipTransform(this["_mcTriangle" + _loc3_],dofus["\r\x14"].gapi.ui.MapExplorer.OVER_TRIANGLE_TRANSFORM);
            break;
         ...
         default:
            var _loc4_ = _loc2_.target._name;
            this.gapi.showTooltip(this.api.lang.getHintsCategory(Number(_loc4_.substr(9,_loc4_.length))).n,_loc2_.target,-20);
      }
   }
   function out(§\x1e\x19\x0e§)
   {
      ...
      {
         var _loc3_ = 0;
         while(_loc3_ < dofus["\r\x14"].gapi.ui.MapExplorer.DIRECTIONS.length)
         {
            this.setMovieClipTransform(this["_mcTriangle" + dofus["\r\x14"].gapi.ui.MapExplorer.DIRECTIONS[_loc3_]],dofus["\r\x14"].gapi.ui.MapExplorer.OUT_TRIANGLE_TRANSFORM);
            _loc3_ = _loc3_ + 1;
         }
      }
   }
   ...
   function zoom(§\x1e\x19\x0e§)
   {
      this._vsZoom.value = _loc2_.target.zoom;
   }
   ...
   function overMap(§\x1e\x19\x0e§)
   {
      if(this.dungeon == undefined)
      {
         var _loc3_ = this.api.kernel.AreasManager.getAreaIDFromCoordinates(_loc2_.coordinates.x,_loc2_.coordinates.y,this._dmMap.superarea);
         var _loc4_ = this.api.kernel.AreasManager.getSubAreaIDFromCoordinates(_loc2_.coordinates.x,_loc2_.coordinates.y,this._dmMap.superarea);
         if(_loc4_ != undefined)
         {
            var _loc5_ = this.api.lang.getMapSubAreaText(_loc4_).n;
            var _loc6_ = dofus.datacenter["\x1e\f\x06"](this.api.datacenter.Subareas.getItemAt(_loc4_));
            if(_loc6_ != undefined)
            {
   ...
   function xtraLayerLoad(§\x1e\x19\x0e§)
   {
   ...
   function worldDataChanged(§\x1e\x19\x0e§)
   {
      if(this["_mcFilter" + dofus["\r\x14"].gapi.ui.MapExplorer.FILTER_CONQUEST_ID].selected)
      {
         this.addToQueue({object:this,method:this.drawHintsOnCategoryLayer,params:[dofus["\r\x14"].gapi.ui.MapExplorer.FILTER_CONQUEST_ID]});
      }
   }
