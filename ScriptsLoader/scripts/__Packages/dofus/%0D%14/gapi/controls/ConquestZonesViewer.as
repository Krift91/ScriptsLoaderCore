class dofus.§\r\x14§.gapi.controls.ConquestZonesViewer extends dofus.§\r\x14§.gapi.core.§\x10\x19§
{
   var _lblFilter;
   var _lblAreas;
   var _lblAreaTitle;
   ...
   function init()
   {
      super.init(false,dofus["\r\x14"].gapi.controls.ConquestZonesViewer.CLASS_NAME);
   }
   ...
   function initTexts()
   {
      ...
      this._lblAreas.text = ank["\x1e\n\x07"]["\x1e\x16\x19"].combine(this.api.lang.getText("CONQUEST_AREA_WORD"),null,false);
      this._lblAreaTitle.text = ank["\x1e\n\x07"]["\x1e\x16\x19"].combine(this.api.lang.getText("CONQUEST_AREA_WORD"),null,true);
      this._lblAreaDetails.text = this.api.lang.getText("CONQUEST_STATE_WORD") + " / " + this.api.lang.getText("CONQUEST_PRISM_WORD");
      this._lblVillages.text = ank["\x1e\n\x07"]["\x1e\x16\x19"].combine(this.api.lang.getText("CONQUEST_VILLAGE_WORD"),null,false);
      this._lblVillageTitle.text = ank["\x1e\n\x07"]["\x1e\x16\x19"].combine(this.api.lang.getText("CONQUEST_VILLAGE_WORD"),null,true);
   ...
   function refreshAreaList()
   {
      ...
      {
         if(!(_loc3_ == dofus["\r\x14"].gapi.controls.ConquestZonesViewer.FILTER_HOSTILE_AREAS && !_loc2_.areas[_loc6_].fighting))
         {
            if(!(_loc3_ == dofus["\r\x14"].gapi.controls.ConquestZonesViewer.FILTER_CAPTURABLE_AREAS && !_loc2_.areas[_loc6_].isCapturable()))
            {
               if(!(_loc3_ == dofus["\r\x14"].gapi.controls.ConquestZonesViewer.FILTER_VULNERALE_AREAS && !_loc2_.areas[_loc6_].isVulnerable()))
               {
   ...
   {
      var _loc2_ = this.api.datacenter.Conquest.worldDatas;
      this._lblGotAreas.text = ank["\x1e\n\x07"]["\x1e\x16\x19"].combine(this.api.lang.getText("CONQUEST_POSSESSED_WORD"),"f",false) + " : " + _loc2_.ownedAreas + " / " + _loc2_.possibleAreas + " / " + _loc2_.totalAreas;
      this._lblGotVillages.text = ank["\x1e\n\x07"]["\x1e\x16\x19"].combine(this.api.lang.getText("CONQUEST_POSSESSED_WORD"),"m",false) + " : " + _loc2_.ownedVillages + " / " + _loc2_.totalVillages;
      ...
      _loc3_.push({label:this.api.lang.getText("CONQUEST_HOSTILE_AREAS"),value:dofus["\r\x14"].gapi.controls.ConquestZonesViewer.FILTER_HOSTILE_AREAS});
      _loc3_.push({label:this.api.lang.getText("CONQUEST_CAPTURABLE_AREAS"),value:dofus["\r\x14"].gapi.controls.ConquestZonesViewer.FILTER_CAPTURABLE_AREAS});
      _loc3_.push({label:this.api.lang.getText("CONQUEST_VULNERALE_AREAS"),value:dofus["\r\x14"].gapi.controls.ConquestZonesViewer.FILTER_VULNERALE_AREAS});
      _loc3_.push({label:this.api.lang.getText("CONQUEST_ALL_AREAS"),value:dofus["\r\x14"].gapi.controls.ConquestZonesViewer.FILTER_ALL_AREAS});
   ...
   function worldDataChanged(§\x0f\x04§)
   {
      this.addToQueue({object:this,method:this.initData});
   }
   function itemSelected(§\x0f\x04§)
   {
