class dofus.§\r\x14§.gapi.controls.StarsDisplayer extends dofus.§\r\x14§.gapi.core.§\x10\x19§
{
   var _nValue;
   var _mcMask;
   ...
   function init()
   {
      super.init(false,dofus["\r\x14"].gapi.controls.StarsDisplayer.CLASS_NAME);
   }
   ...
   function updateData()
   {
      if(this._nValue != undefined && (this._nValue > 0 && !_global.isNaN(this._nValue)))
      {
         ...
         var _loc3_ = 0;
         while(_loc3_ < dofus["\r\x14"].gapi.controls.StarsDisplayer.STARS_COUNT)
         {
      ...
      {
         var _loc6_ = 0;
         while(_loc6_ < dofus["\r\x14"].gapi.controls.StarsDisplayer.STARS_COUNT)
         {
   ...
   function getStarsColor()
   {
      ...
      while(_loc3_ < dofus["\r\x14"].gapi.controls.StarsDisplayer.STARS_COUNT)
      {
         var _loc4_ = Math.floor(this._nValue / 100) + (this._nValue - Math.floor(this._nValue / 100) * 100 <= _loc3_ * (100 / dofus["\r\x14"].gapi.controls.StarsDisplayer.STARS_COUNT) ? 0 : 1);
         _loc2_[_loc3_] = dofus["\r\x14"].gapi.controls.StarsDisplayer.STARS_COLORS[Math.min(_loc4_,dofus["\r\x14"].gapi.controls.StarsDisplayer.STARS_COLORS.length - 1)];
         _loc3_ = _loc3_ + 1;
      }
      return _loc2_;
   }
