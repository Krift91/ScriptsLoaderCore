...
_loc1.setValue = function(_loc2_, _loc3_, _loc4_)
{
   if(_loc2_)
   {
      ...
      {
         var _loc7_ = _loc3_.kernel.OptionsManager.getOption("RemasteredSpellIconsPack");
         this._ldrIcon.content.gotoAndStop(_loc7_);
      }
      var _loc6_ = this._mcList._parent._parent.canBoost(_loc4_) && _loc3_.datacenter.Basics.canUseSeeAllSpell;
      var _loc5_ = this._mcList._parent._parent.getCostForBoost(_loc4_);
      var _loc4_ = _loc4_.level > 1 && _loc3_.datacenter.Basics.canUseSeeAllSpell;
      this._btnBoost.enabled = true;
      this._btnBoost._visible = _loc6_;
      this._btnForget.enabled = true;
      this._btnForget._visible = _loc4_;
      ...
      this._lblBoost.text = _loc5_ != -1 && _loc3_.datacenter.Basics.canUseSeeAllSpell ? _loc3_.lang.getText("POINT_NEED_TO_BOOST_SPELL",[_loc5_]) : "";
      if(this.player.Level < _loc4_._minPlayerLevel)
      {
         _loc4_ = 50;
...
_loc1.click = function(oEvent)
{
   ...
   switch(_loc2_)
   {
      ...
      case this._btnForget:
         _loc3_.network.send2(this.player.ID,"Aps" + this._oItem.ID);
   }
};
