_global.dofus["\r\x14"].battlefield["\x1e\x0b\x05"] = function(sText, sFile, nColor, nFrame, sTitle, nColorTitle, nStarsValue, nExperiencia, nTipoMob)
{
   super();
   this.initialize(nStarsValue,nExperiencia,nTipoMob);
...
_loc1.initialize = function(nStarsValue, nExperiencia, nTipoMob)
{
   super.initialize();
   ...
   {
      nExperiencia = undefined;
   }
   this._sExperiencia = nExperiencia;
   this._nStarsValue = nStarsValue;
   this._sTipoMob = nTipoMob;
   this.createTextField("_txtTitle",1000,0,_global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.HEIGHT_SPACER,0,0);
   if(this._sExperiencia != undefined || this._sTipoMob != undefined)
   {
      var _loc4_ = _global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.HEIGHT_SPACER;
      ...
      this.createTextField("_txtTipoMob",1020,0,_loc4_ + _loc4_ / 2,0,0);
   }
   ...
   this._txtTipoMob.embedFonts = true;
...
_loc1.draw = function(sText, sFile, nColor, nFrame, sTitle, nColorTitle)
{
   ...
   if(this._sExperiencia != undefined)
   {
      ...
      var _loc6_ = "";
      var _loc8_ = 0;
      var _loc4_ = this._sExperiencia.length - 1;
      while(_loc4_ >= 0)
      {
      ...
      this._txtExperiencia.text = _loc6_ + " EXP ESTIMÉ";
   ...
   if(this._sTipoMob != undefined)
   {
      var _loc12_ = this._sTipoMob.split("*");
      this._txtTipoMob.autoSize = "center";
      this._txtTipoMob.text = _loc12_[0];
      this._txtTipoMob.selectable = false;
      this._txtTipoMob.setTextFormat(_global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.TEXT_FORMAT);
      this._txtTipoMob.textColor = Number(_loc12_[1]);
   }
   ...
   var _loc13_ = Math.ceil(Math.max(this._sExperiencia == undefined ? 0 : this._txtExperiencia.textWidth,Math.max(this._txtText.textWidth,this._txtTitle.textWidth,this._nStarsValue <= 0 ? 0 : this._nFullWidth)) + _global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.WIDTH_SPACER * 2);
   var _loc14_ = this._height + _global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.HEIGHT_SPACER;
   if(this._sTipoMob != undefined)
   {
      this.drawBackground(_loc13_,_loc14_,Number(_loc12_[2]));
   }
   else
   {
      this.drawBackground(_loc13_,_loc14_,_global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.BACKGROUND_COLOR);
   }
   var _loc15_ = this.createEmptyMovieClip("StarsDisplayer",this.getNextHighestDepth());
   _loc15_._x = this._width / 2;
   _loc15_._y = this._height + _global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.HEIGHT_SPACER;
   if(this._nStarsValue > 0)
   {
      var _loc10_ = this.getStarsColor();
      var _loc3_ = 0;
      var _loc11_ = _global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.HEIGHT_SPACER;
      ...
      while(_loc3_ < dofus["\r\x14"].battlefield["\x1e\x0b\x05"].STARS_COUNT)
      {
         ...
         var _loc7_ = this._aStars[_loc3_].star.fill;
         if(_loc10_[_loc3_] > -1)
         {
            var _loc9_ = new Color(_loc7_);
...
_loc1.getStarsColor = function()
{
   ...
   {
      var _loc3_ = Math.floor(this._nStarsValue / 100) + (this._nStarsValue - Math.floor(this._nStarsValue / 100) * 100 <= _loc2_ * (100 / dofus["\r\x14"].battlefield["\x1e\x0b\x05"].STARS_COUNT) ? 0 : 1);
