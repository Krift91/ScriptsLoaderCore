...
_global.dofus["\r\x14"].battlefield["\r\f"] = function(sText, sSpriteName, oEmblem, sFile, nFrame, nPvpGain, title, sFile2, nOrnamento, nPrestige, nNivel, RealPrestige)
{
   super();
   this.initialize(title != undefined);
   this.draw(sText,sSpriteName,oEmblem,sFile,nFrame,nPvpGain,title,sFile2,nOrnamento,nPrestige,nNivel,RealPrestige);
};
...
_loc1.draw = function(sGuildName, sSpriteName, oEmblem, sFile, nFrame, nPvpGain, title, sFile2, nOrnamento, nPrestige, nNivel, RealPrestige)
{
   ...
   if(nPrestige == 1)
   {
      ...
      var _loc7_ = false;
      var _loc10_ = false;
      var _loc9_ = new flash.filters.GlowFilter(15063833,_loc14_,_loc3_,_loc3_,_loc12_,_loc17_,_loc7_,_loc10_);
   ...
   if(title)
   {
      ...
      _loc11_ = _global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.HEIGHT_SPACER + this._txtTitle.textHeight;
      this._txtGuildName._x = this._txtSpriteName._x = (- _loc3_) / 2 + 30 + _global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.WIDTH_SPACER * 2;
   ...
   var _loc16_ = _global.API.ui.getUIComponent("Banner");
   _loc16_.drawPres(RealPrestige,nNivel);
};
