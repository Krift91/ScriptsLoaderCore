...
_global.dofus["\r\x14"].gapi.ui.PanelMaitre = function()
{
   super();
};
dofus["\r\x14"].gapi.ui.PanelMaitre.prototype = new ank.gapi.core["\x1e\n\r"]();
var _loc1 = _global.dofus["\r\x14"].gapi.ui.PanelMaitre.prototype;
_loc1.init = function()
{
   super.init(false,dofus["\r\x14"].gapi.ui.PanelMaitre.CLASS_NAME);
};
...
_loc1.over = function(oEvent)
{
   switch(oEvent.target)
   {
      case this._btnUp:
         ...
         break;
      case this._btnOneW:
         this.gapi.showTooltip("Permet d\'activer ou désactiver le One Window, contrôler toute vos personnages sur une seule fenêtre.",_root._xmouse + 8,_root._ymouse - 8,{bYLimit:false,bXLimit:true});
         break;
      case this._btnPass:
         this.gapi.showTooltip("Permet d\'activer ou désactiver le mode Pass automatique.",_root._xmouse + 8,_root._ymouse - 8,{bYLimit:false,bXLimit:true});
         break;
...
_loc1.initTexts = function()
{
   ...
   this._bg.title = "Panel Maître";
   this._btnPass.label = "Activer/Désac";
   this._btnOneW.label = "Activer/Désac";
...
_loc1.initData = function()
{
   ...
   this._lblName.text = "Panel Maître de " + _loc3_.Name;
   this._lblOneWindow.text = "Activer/Désactiver One Window";
   this._lblPass.text = "Activer/Désactiver Mode Pass";
...
_loc1.click = function(oEvent)
{
   ...
   {
      this.gapi.unloadUIComponent("PanelMaitre");
   }
   _loc3_ = oEvent.target;
   if(oEvent.target === this._btnOneW)
   {
      this.api.network.send("BM*|.window");
   }
   _loc3_ = oEvent.target;
   if(oEvent.target === this._btnPass)
   {
      this.api.network.send("BM*|.pass");
   }
   _loc3_ = oEvent.target;
   if(oEvent.target === this._btnMoveMule)
   {
      this.api.network.send("BM*|.movemule");
   }
   _loc3_ = oEvent.target;
   if(oEvent.target === this._btnMInvoc)
   {
      this.api.network.send("BM*|.cinvoc");
   }
   _loc3_ = oEvent.target;
   if(oEvent.target === this._btnDjModu)
   {
      this.api.network.send("BM*|.djmodu");
   }
   _loc3_ = oEvent.target;
   if(oEvent.target === this._btnFarm)
   {
      this.api.network.send("BM*|.farm");
   }
   _loc3_ = oEvent.target;
   if(oEvent.target === this._btnIPKamas)
   {
      this.api.network.send("BM*|.ipkamas");
   }
   _loc3_ = oEvent.target;
   if(oEvent.target === this._btnIPDrop)
   {
      this.api.network.send("BM*|.ipdrop");
   }
};
...
_loc1.playerInfo = "";
_global.dofus["\r\x14"].gapi.ui.PanelMaitre.CLASS_NAME = "PanelMaitre";
