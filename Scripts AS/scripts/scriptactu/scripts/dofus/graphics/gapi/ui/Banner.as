   ...
   if(_loc7_ !== this._mcbMovableBar)
   {
      ...
      if(this._msShortcuts.currentTab !== "Spells")
      {
         if(_loc7_ === "Items")
         {
            ...
            {
               var _loc11_ = this.gapi.loadUIComponent("PopupQuantity","PopupQuantity",{value:_loc4_.Quantity,max:_loc4_.Quantity,useAllStage:true,params:{type:"drop",item:_loc4_,position:_loc8_}},{bAlwaysOnTop:true});
      ...
      else
      {
         ...
         var _loc9_ = this._msShortcuts.getContainer(_loc5_).contentData;
         if(_loc9_ != undefined)
         {
...
_loc1.addListeners = function()
{
   ...
   this._btnSonge.addEventListener("click",this);
   this._btnZaap.addEventListener("click",this);
   ...
   this._btnPeleas.addEventListener("click",this);
   ...
   this._btnPeleas.addEventListener("over",this);
   ...
   this._btnPeleas.addEventListener("out",this);
...
_loc1.mostrarBotones = function()
{
   ...
   this._btnSonge._visible = this.api.datacenter.Game.isFight == false;
   this._btnZaap._visible = this.api.datacenter.Game.isFight == false;
   ...
   this._btnShow2._visible = this.api.datacenter.Game.isFight == false;
   ...
   this._btnCasino._visible = this.api.datacenter.Game.isFight == false;
   this._btnFarming._visible = this.api.datacenter.Game.isFight == false;
   ...
   this._cChat._btnTitre._visible = this.api.datacenter.Game.isFight == false;
   this._btnTitre._visible = this.api.datacenter.Game.isFight == false;
   ...
   this._cChat._btnCasino._visible = this.api.datacenter.Game.isFight == false;
   this._cChat._btnFarming._visible = this.api.datacenter.Game.isFight == false;
   this._cChat._btnHeroes._visible = this.api.datacenter.Game.isFight == false;
   this._btnHeroes._visible = this.api.datacenter.Game.isFight == false;
   this._cChat._btnPerso._visible = this.api.datacenter.Game.isFight == false;
   this._btnPerso._visible = this.api.datacenter.Game.isFight == false;
   this._cChat._btnJoueur._visible = this.api.datacenter.Game.isFight == false;
   this._btnJoueur._visible = this.api.datacenter.Game.isFight == false;
};
...
_loc1.over = function(_loc2_)
{
   ...
   switch(_loc2_.target._name)
   {
      ...
      case "_btnFriends":
         ...
         break;
      case "_btnPeleas":
         this.gapi.showTooltip(this.api.lang.getText("FIGHTS_ON_SERVER"),_loc2_.target,-20,{bXLimit:true,bYLimit:false});
         break;
      case "_btnNextTurn":
...
_loc1.click = function(oEvent)
{
   ...
   switch(oEvent.target._name)
   {
      ...
      case "_btnOneWindows":
         ...
         break;
      case "_btnSonge":
         this.gapi.loadUIAutoHideComponent("PanelSonge","PanelSonge");
         this.api.network.send("ko");
         break;
      case "_btnHardcore":
         this.api.network.send("XDJ");
         break;
      case "_btnZaap":
         this.api.network.send("XDK");
         break;
      case "_btnAntiLagTp":
      ...
      case "_btnPeleas":
         if(!this.api.datacenter.Game.isFight)
         {
            this.api.network.send("fG");
         }
         break;
      case "_btnHelp":
         var _loc7_ = this.api.lang.getConfigText("CHAT_FILTERS");
         ...
         _loc3_.addItem("Naeris (.all)",this,this.setChatPrefix,[".all"]);
...
_loc1.drawPres = function(Prestige, Level)
{
   if(!this.api.datacenter.Game.isFight)
   {
      if(Prestige != undefined)
      {
         this._lPres._visible = true;
         this._srPres._visible = true;
         this._lblPres._visible = true;
         this._lblNivel._visible = true;
         this._lblPres.text = "Prestige " + Prestige;
         if(Number(Level) > 200)
         {
            this._lblNivel.text = "Oméga " + Number(Level - 200);
         }
         else
         {
            this._lblNivel.text = "Niveau " + Number(Level);
         }
         this._lPres.contentPath = "data/files/prestiges/" + Prestige + ".swf";
      }
      else
      {
         this._lPres._visible = false;
         this._srPres._visible = false;
         this._lblPres._visible = false;
         this._lblNivel._visible = false;
      }
   }
};
_loc1.href = function(oEvent, oCustomPopupPosition)
{
   var _loc5_ = oEvent.params.split(",");
   switch(_loc5_[0])
   {
      case "showEndPanel":
         this.addToQueue({object:this.api.kernel.GameManager,method:this.api.kernel.GameManager.showEndPanel,params:[_loc5_[1]]});
         break;
      ...
      case "OpenGuildTaxCollectors":
         var _loc11_ = this.gapi.getUIComponent("Guild");
         if(_loc11_ == null)
         {
         ...
         {
            this.addToQueue({object:_loc11_,method:_loc11_.setCurrentTab,params:["TaxCollectors"]});
         }
         break;
      ...
      case "ShowPlayerPopupMenu":
         if(_loc5_[2] != undefined && (String(_loc5_[2]).length > 0 && _loc5_[2] != ""))
         {
            this.addToQueue({object:this.api.kernel.GameManager,method:this.api.kernel.GameManager.showPlayerPopupMenu,params:[undefined,_global.unescape(_loc5_[1]),null,null,null,_loc5_[2],this.api.datacenter.Player.isAuthorized]});
         }
         else
         {
            this.addToQueue({object:this.api.kernel.GameManager,method:this.api.kernel.GameManager.showPlayerPopupMenu,params:[undefined,_global.unescape(_loc5_[1]),null,null,null,null,this.api.datacenter.Player.isAuthorized]});
         }
         break;
      case "ShowMessagePopupMenu":
         if(_loc5_[3] != undefined && (String(_loc5_[3]).length > 0 && _loc5_[3] != ""))
         {
            this.addToQueue({object:this.api.kernel.GameManager,method:this.api.kernel.GameManager.showMessagePopupMenu,params:[_loc5_[1],_global.unescape(_loc5_[2]),_loc5_[3]]});
         }
         else
         {
            this.addToQueue({object:this.api.kernel.GameManager,method:this.api.kernel.GameManager.showMessagePopupMenu,params:[_loc5_[1],_global.unescape(_loc5_[2])]});
         }
         break;
      case "ShowItemViewer":
         var _loc12_ = this.api.kernel.ChatManager.getItemFromBuffer(Number(_loc5_[1]));
         ...
         this.addToQueue({object:this.api.ui,method:this.api.ui.loadUIComponent,params:["ItemViewer","ItemViewer",{item:_loc12_},{bAlwaysOnTop:true}]});
         break;
      case "updateCompass":
         this.api.kernel.GameManager.updateCompass(Number(_loc5_[1]),Number(_loc5_[2]));
         break;
      case "ShowLinkWarning":
         this.addToQueue({object:this.api.ui,method:this.api.ui.loadUIComponent,params:["AskLinkWarning","AskLinkWarning",{text:this.api.lang.getText(_loc5_[1])}]});
         break;
      case "highlightSprite":
         ...
         while(_loc3_ < _loc5_.length)
         {
            ...
            var _loc4_ = this.api.datacenter.Sprites.getItemAt(_loc6_);
            if(_loc4_ != undefined)
            {
               if(!(!_loc4_.isVisible || _loc4_.isClear))
               {
                  var _loc7_ = _loc4_.cellNum;
                  if(_loc7_ != undefined)
                  {
                     this.addToQueue({object:this.api.gfx.mapHandler,method:this.api.gfx.mapHandler.flagCellNonBlocking,params:[_loc7_,_loc6_]});
                  }
               }
            }
            _loc3_ += 1;
