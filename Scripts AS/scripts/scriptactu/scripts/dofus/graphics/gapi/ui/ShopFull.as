...
_loc1.__set__objetos = function(sData)
{
   ...
   while(_loc3_ < _loc10_.length)
   {
      ...
      var _loc4_ = _loc2_.split("@")[1];
      var _loc9_ = _loc2_.split("@")[0];
      var _loc7_ = _loc2_.split("@")[2];
      var _loc6_ = _loc2_.split("@")[3] == 1;
      var _loc8_ = _loc2_.split("@")[4] == 1;
      var _loc5_ = new dofus.datacenter["\f\f"](0,Number(_loc4_),1,-1,_loc2_.split("@")[5].split("^").join(","));
...
_loc1.addTabListeners = function()
{
   ...
   while(_loc2_ < 39)
   {
      var _loc4_ = this._mcTabViewer["shop" + _loc2_].cuadro;
      ...
      var _loc3_ = this._mcTabViewer["_prevItem" + _loc2_];
...
_loc1.initTexts = function()
{
   ...
   this._btnComprar.label = this.api.lang.getText("COMPRAR_OGR");
...
_loc1.initData = function()
{
   ...
   while(_loc4_ < 39)
   {
      var _loc10_ = this._mcTabViewer["shop" + _loc4_];
      var _loc9_ = this._mcTabViewer["card" + _loc4_];
      var _loc11_ = this._mcTabViewer["_prevItem" + _loc4_];
   ...
   {
      var _loc3_ = this._oObjetos[_loc16_];
      if(_loc3_.tipo == this.sCategory)
      {
         ...
         var _loc5_ = new dofus.datacenter["\f\f"](0,Number(_loc3_.id));
   ...
   if(this._itemsCart.length > 0)
   {
      ...
      {
         var _loc12_ = this._itemsCart[_loc6_];
         var _loc8_ = Number(_loc12_.cant);
         _loc13_ += _loc8_;
         _loc6_ = _loc6_ + 1;
      }
...
_loc1.descriItem = function(oEvent)
{
   ...
   {
      var _loc3_ = _loc4_.effects[_loc6_];
...
_loc1.updateItemsFilter = function(sFilter)
{
   ...
   while(_loc4_ < 39)
   {
      var _loc7_ = this._mcTabViewer["shop" + _loc4_];
      var _loc8_ = this._mcTabViewer["card" + _loc4_];
      var _loc9_ = this._mcTabViewer["_prevItem" + _loc4_];
   ...
   {
      var _loc3_ = this._oObjetos[_loc12_];
...
_loc1.askBuy = function(_loc3_)
{
   ...
   if(_loc3_ != undefined)
   {
      ...
      {
         var _loc5_ = this.gapi.loadUIComponent("AskYesNo","AskYesNoBuy",{title:this.api.lang.getText("BIGSTORE"),text:this.api.lang.getText("DO_U_BUY_ITEM_BIGSTORE_OGRINAS",[_loc3_.precio * _loc3_ + " Ogrine",_loc3_]),params:{id:_loc3_.id,precio:_loc3_.precio * _loc3_,promo:false,cantidad:_loc3_}});
...
_loc1.click = function(oEvent)
{
   switch(oEvent.target._name)
   {
      ...
      case "_btnSearch":
         ...
         break;
      case "_prevItem0":
         this.api.network.send("kb+" + this.prevItem0.unicID);
         break;
      case "_prevItem1":
         this.api.network.send("kb+" + this.prevItem1.unicID);
         break;
      case "_prevItem2":
         this.api.network.send("kb+" + this.prevItem2.unicID);
         break;
      case "_prevItem3":
         this.api.network.send("kb+" + this.prevItem3.unicID);
         break;
      case "_prevItem4":
         this.api.network.send("kb+" + this.prevItem4.unicID);
         break;
      case "_prevItem5":
         this.api.network.send("kb+" + this.prevItem5.unicID);
         break;
      case "_prevItem6":
         this.api.network.send("kb+" + this.prevItem6.unicID);
         break;
      case "_prevItem7":
         this.api.network.send("kb+" + this.prevItem7.unicID);
         break;
      case "_prevItem8":
         this.api.network.send("kb+" + this.prevItem8.unicID);
         break;
      case "_prevItem9":
         this.api.network.send("kb+" + this.prevItem9.unicID);
         break;
      case "_prevItem10":
         this.api.network.send("kb+" + this.prevItem10.unicID);
         break;
      case "_prevItem11":
         this.api.network.send("kb+" + this.prevItem11.unicID);
         break;
      case "_prevItem12":
         this.api.network.send("kb+" + this.prevItem12.unicID);
         break;
      case "_prevItem13":
         this.api.network.send("kb+" + this.prevItem13.unicID);
         break;
      case "_prevItem14":
         this.api.network.send("kb+" + this.prevItem14.unicID);
         break;
      case "_prevItem15":
         this.api.network.send("kb+" + this.prevItem15.unicID);
         break;
      case "_prevItem16":
         this.api.network.send("kb+" + this.prevItem16.unicID);
         break;
      case "_prevItem17":
         this.api.network.send("kb+" + this.prevItem17.unicID);
         break;
      case "_prevItem18":
         this.api.network.send("kb+" + this.prevItem18.unicID);
         break;
      case "_prevItem19":
         this.api.network.send("kb+" + this.prevItem19.unicID);
         break;
      case "_prevItem20":
         this.api.network.send("kb+" + this.prevItem20.unicID);
         break;
      case "_prevItem21":
         this.api.network.send("kb+" + this.prevItem21.unicID);
         break;
      case "_prevItem22":
         this.api.network.send("kb+" + this.prevItem22.unicID);
         break;
      case "_prevItem23":
         this.api.network.send("kb+" + this.prevItem23.unicID);
         break;
      case "_prevItem24":
         this.api.network.send("kb+" + this.prevItem24.unicID);
         break;
      case "_prevItem25":
         this.api.network.send("kb+" + this.prevItem25.unicID);
         break;
      case "_prevItem26":
         this.api.network.send("kb+" + this.prevItem26.unicID);
         break;
      case "_prevItem27":
         this.api.network.send("kb+" + this.prevItem27.unicID);
         break;
      case "_prevItem28":
         this.api.network.send("kb+" + this.prevItem28.unicID);
         break;
      case "_prevItem29":
         this.api.network.send("kb+" + this.prevItem29.unicID);
         break;
      case "_prevItem30":
         this.api.network.send("kb+" + this.prevItem30.unicID);
         break;
      case "_prevItem31":
         this.api.network.send("kb+" + this.prevItem31.unicID);
         break;
      case "_prevItem32":
         this.api.network.send("kb+" + this.prevItem32.unicID);
         break;
      case "_prevItem33":
         this.api.network.send("kb+" + this.prevItem33.unicID);
         break;
      case "_prevItem34":
         this.api.network.send("kb+" + this.prevItem34.unicID);
         break;
      case "_prevItem35":
         this.api.network.send("kb+" + this.prevItem35.unicID);
         break;
      case "_prevItem36":
         this.api.network.send("kb+" + this.prevItem36.unicID);
         break;
      case "_prevItem37":
         this.api.network.send("kb+" + this.prevItem37.unicID);
         break;
      case "_prevItem38":
         this.api.network.send("kb+" + this.prevItem38.unicID);
         break;
      case "_btnClose":
         this.callClose();
   }
...
_loc1.over = function(oEvent)
{
   switch(oEvent.target._name)
   {
      ...
      case "_prevItem51":
         ...
         break;
      case "_btnComprar":
         this.gapi.showTooltip(this.api.lang.getText("COMPRAR_OGRINAS"),_root._xmouse + 8,_root._ymouse - 8,{bYLimit:false,bXLimit:true});
         break;
      default:
