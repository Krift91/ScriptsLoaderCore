...
_loc1.addListeners = function()
{
   ...
   this._btnStack.addEventListener("click",this);
   this._btnStack.addEventListener("over",this);
   this._btnStack.addEventListener("out",this);
   this._btnCInvoc.addEventListener("click",this);
   this._btnCInvoc.addEventListener("over",this);
   this._btnCInvoc.addEventListener("out",this);
   this._btnAutoAggro.addEventListener("click",this);
   this._btnAutoAggro.addEventListener("over",this);
   this._btnAutoAggro.addEventListener("out",this);
...
_loc1.click = function(oEvent)
{
   switch(oEvent.target)
   {
      ...
      case this._btnStack:
         ...
         break;
      case this._btnCInvoc:
         this.escojerComando(8);
         break;
      case this._btnAutoAggro:
         this.escojerComando(9);
   }
};
...
_loc1.over = function(oEvent)
{
   ...
   {
      case "_btnTienda":
         this.gapi.showTooltip("Boutique",oEvent.target,-20,{bYLimit:false,bXLimit:true});
         break;
      case "_btnBoton2":
         this.gapi.showTooltip("Ma Banque",oEvent.target,-20,{bYLimit:false,bXLimit:true});
         break;
      case "_btnTurno":
      ...
      case "_btnDj":
         ...
         break;
      case "_btnStack":
         this.gapi.showTooltip(this.api.lang.getText("ACTIVE_DESACTIVE_STACK"),oEvent.target,-20,{bYLimit:false,bXLimit:true});
         break;
      case "_btnCInvoc":
         this.gapi.showTooltip(this.api.lang.getText("ACTIVE_DESACTIVE_CINVOC"),oEvent.target,-20,{bYLimit:false,bXLimit:true});
         break;
      case "_btnAutoAggro":
         this.gapi.showTooltip(this.api.lang.getText("ACTIVE_DESACTIVE_AUTO"),oEvent.target,-20,{bYLimit:false,bXLimit:true});
   }
};
_loc1.initTexts = function()
{
   ...
   this._btnTienda.label = "Boutique";
   this._btnBoton2.label = "Ma Banque";
};
...
_loc1.cargarDatos = function(sValue)
{
   ...
   this._txtPocionExp.text = Number(_loc2_[0]) >= 1 ? String(_loc2_[0]) : "Inactif";
   ...
   this._txtTempsJeu.text = Number(_loc2_[15]) >= 1 ? String(_loc2_[15]) : "Jamais";
   ...
   var _loc7_ = Number(_loc2_[8]);
   var _loc3_ = 0;
   while(_loc3_ < 5)
   {
      _loc6_.push({id:_loc3_,label:"Level " + _loc3_});
      _loc3_ = _loc3_ + 1;
   }
   this._cbHard.dataProvider = _loc6_;
   this._cbHard.selectedIndex = _loc7_;
   this._txtHm.text = "Level: " + Number(_loc2_[8]);
   this._txtNFight.text = Number(_loc2_[9]);
   this._txtAd2.text = Number(_loc2_[10]);
   this._txtAd3.text = Number(_loc2_[11]);
   this._txtStack.text = Number(_loc2_[12]) != 0 ? "Actif" : "Inactif";
   if(Number(_loc2_[12]) == 0)
   {
      this._btnStack.styleName = "RedCheckButton";
      this._btnStack.selected = false;
   }
   else if(Number(_loc2_[12]) != 0)
   {
      this._btnStack.styleName = "GreenCheckButton";
      this._btnStack.selected = true;
   }
   this._txtCInvoc.text = Number(_loc2_[13]) != 0 ? "Actif" : "Inactif";
   if(Number(_loc2_[13]) == 0)
   {
      this._btnCInvoc.styleName = "RedCheckButton";
      this._btnCInvoc.selected = false;
   }
   else if(Number(_loc2_[13]) != 0)
   {
      this._btnCInvoc.styleName = "GreenCheckButton";
      this._btnCInvoc.selected = true;
   }
   this._txtAutoAggro.text = Number(_loc2_[14]) != 0 ? "Actif" : "Inactif";
   if(Number(_loc2_[14]) == 0)
   {
      this._btnAutoAggro.styleName = "RedCheckButton";
      this._btnAutoAggro.selected = false;
   }
   else if(Number(_loc2_[14]) != 0)
   {
      this._btnAutoAggro.styleName = "GreenCheckButton";
      this._btnAutoAggro.selected = true;
   }
};
_loc1.itemSelected = function(oEvent)
{
   var _loc0_ = null;
   if((_loc0_ = oEvent.target._name) === "_cbHard")
   {
      if(this._cbHard.selectedItem.id == 0)
      {
         this.escojerHard(0);
      }
      if(this._cbHard.selectedItem.id == 1)
      {
         this.escojerHard(1);
      }
      if(this._cbHard.selectedItem.id == 2)
      {
         this.escojerHard(2);
      }
      if(this._cbHard.selectedItem.id == 3)
      {
         this.escojerHard(3);
      }
      if(this._cbHard.selectedItem.id == 4)
      {
         this.escojerHard(4);
      }
   }
...
_loc1.initPlayer = function()
{
   ...
   var _loc3_ = new ank.battlefield.datacenter["\x1e\x0e\x10"](undefined,ank.battlefield.mc["\x1e\x0e\x10"],_global.palmad.dofus.Constants.CLIPS_PERSOS_PATH + _loc5_ + ".swf",undefined,5);
   _loc3_.accessories = _loc4_.accessories;
   this._svCharacter.enableBlur = true;
   this._svCharacter.refreshAccessories = !var2;
   this._svCharacter.sourceSpriteData = _loc4_;
   this._svCharacter.spriteData = _loc3_;
