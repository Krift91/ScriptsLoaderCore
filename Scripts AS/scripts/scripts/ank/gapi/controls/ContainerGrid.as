...
_loc1.layoutContent = function()
{
   ...
   if(!this._bInvalidateLayout)
   {
      ...
      while(_loc10_ < this._nVisibleRowCount)
      {
         ...
         {
            var _loc3_ = this._mcScrollContent["c" + _loc4_];
            if(_loc3_ == undefined)
            {
   ...
   while(_loc13_ < this._nVisibleRowCount)
   {
      ...
      {
         var _loc5_ = this._mcScrollContent["c" + _loc11_];
         var _loc6_ = this._eaDataProvider[_loc4_];
         var _loc12_ = _loc6_.label;
         var _loc9_ = _loc6_.forceReloadOnContainer;
...
{
   var _loc2_ = this.getSelectedItems();
   this.dispatchEvent({type:"dblClickItem",target:_loc2_.target,targets:_loc2_,owner:this,item:_loc2_.target.contentData});
};
