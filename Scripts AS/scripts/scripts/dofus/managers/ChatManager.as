...
_loc1.parseInlineItems = function(sMessage, aItems)
{
   ...
   while(true)
   {
      ...
      var _loc5_ = aItems[_loc2_].split(";");
      var _loc10_ = Number(_loc5_[0]);
      var _loc13_ = Number(_loc5_[1]);
      var _loc11_ = aItems[_loc2_ + 1];
      var _loc9_ = new dofus.datacenter["\f\f"](_loc13_,_loc10_,1,1,_loc11_,-1);
      var _loc3_ = "°" + _loc2_ / 2;
      var _loc8_ = this.getLinkItem(_loc9_);
      var _loc4_ = sMessage.indexOf(_loc3_);
      if(_loc4_ != -1)
      {
         var _loc6_ = sMessage.split("");
...
_global.dofus["\x0b\b"].ChatManager.safeHtml = function(s)
{
   ...
   {
      var _loc4_ = _loc1_.indexOf("<");
      if(_loc1_.indexOf(">",_loc4_) == -1)
      {
         _loc1_ = _loc1_.split("<").join("&lt;").split(">").join("&gt;");
         break;
      }
      var _loc6_ = _loc1_.indexOf(">",_loc4_) + 1;
      var _loc2_ = _loc1_.substring(_loc4_,_loc6_);
      var _loc5_ = _loc2_.indexOf("/") == 1;
      var _loc8_ = _loc2_.indexOf("/") == _loc2_.length - 2;
      var _loc3_ = !!_loc5_ ? _loc2_.substring(2,_loc2_.length - 1) : _loc2_.substring(1,_loc2_.length - 1);
...
_loc1.refresh = function(_loc2_)
{
   ...
   while(_loc5_ < dofus["\x0b\b"].ChatManager.MAX_VISIBLE && _loc3_ >= 0)
   {
      ...
      if(this._aVisibleTypes[_loc2_.type] == true)
      {
         ...
         if(!_loc2_.htmlSyntaxChecked)
         {
            var _loc6_ = dofus["\x0b\b"].ChatManager.safeHtml(_loc2_.text);
            _loc2_.lf = _loc2_.lf;
