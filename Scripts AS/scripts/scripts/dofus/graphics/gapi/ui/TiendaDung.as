...
_loc1.valid = function(noConfirm, id)
{
   ...
   if(!noConfirm)
   {
      this.api.kernel.showMessage("ATTENTION","x" + this["_nPrecioID" + id] + " (" + this.api.lang.getItemUnicText(this._nItemFichaID).n + ") seront déduits pour acheter x" + this["_nCantidadID" + id] + " (" + this.api.lang.getItemUnicText(this["_nItemID" + id]).n + "). Voulez-vous continuer ?","CAUTION_YESNO",{name:"Confirm",listener:this});
      return undefined;
   }
   var _loc0_ = null;
   if((_loc0_ = this._oEvent.target) === this["_btnReclamar" + id])
   {
      this.api.network.send("MR" + this._nItemFichaID + "|" + this["_nPrecioID" + id] + "|" + this["_nItemID" + id] + "|" + this["_nCantidadID" + id] + "|" + this._nRetrato);
   }
};
