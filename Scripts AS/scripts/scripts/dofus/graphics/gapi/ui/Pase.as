...
_loc1.cargaTodo = function(contenido, elvalxx)
{
   ...
   var _loc8_ = this._datos.split(",")[_loc17_].split(";")[2];
   var _loc19_ = this._datos.split(",")[_loc17_].split(";")[3];
   if(elvalxx)
   {
      this.desliza = int(_loc19_);
   }
   var _loc20_ = this._datos.split(",")[_loc17_].split(";")[4];
   var _loc18_ = this._datos.split(",")[_loc17_].split(";")[5];
   this.mispts = int(_loc18_);
   if(_loc19_ == 0)
   {
   ...
   this._lblPuntos.text = int(_loc18_);
   if(_loc4_.length <= 8 || _loc20_ == 1)
   {
   ...
   while(_loc3_ < 8)
   {
      ...
      var _loc15_ = _loc4_[_loc3_].split(";")[5];
      var _loc7_ = _loc4_[_loc3_].split(";")[8];
      this["item" + _loc2_] = _loc15_;
      if(_loc10_ == "item")
      {
         var _loc9_ = new ank["\x1e\n\x07"]["\x0f\x01"]();
         var _loc5_ = _loc4_[_loc3_].split(";")[7];
         var _loc14_ = _loc5_.split("^").join(",");
         var _loc12_ = new dofus.datacenter["\f\f"](_loc7_,_loc7_,1,-1,_loc14_);
         _loc9_.push(_loc12_);
      ...
      {
         var _loc11_ = new dofus.datacenter["\x1e\x0f\x02"](_loc7_,1);
      ...
      var _loc16_ = _loc4_[_loc3_].split(";")[3];
      this["precio" + _loc2_].text = _loc16_;
      var _loc13_ = _loc4_[_loc3_].split(";")[4];
      ...
      {
         if(_loc5_ == 1 && _loc8_ == 1 || _loc5_ == 0 && _loc8_ == 1 || _loc5_ == 0 && _loc8_ == 0)
         {
            this["reclama" + _loc2_]._visible = true;
            this["reclama" + _loc2_].enabled = true;
         }
         else if(_loc5_ == 1 && _loc8_ == 0)
         {
            this["reclama" + _loc2_]._visible = true;
            this["reclama" + _loc2_]._visible = false;
         }
...
_loc1.initTexts = function()
{
   ...
   this.infoventana.title = "Passe de Combat";
...
_loc1.click = function(oEvent)
{
   switch(oEvent.target)
   {
      case this.btnDer:
         ...
         this.recibe = true;
         this.desliza += 8;
         _global.API.network.send("!CPA" + this.desliza);
         break;
      case this.btnIza:
         if(this.recibe)
         {
      ...
      case this.reclama1:
         ...
         _global.API.network.send("!CPO" + this.item1);
         break;
      case this.reclama2:
         this.reclama2._visible = false;
         _global.API.network.send("!CPO" + this.item2);
         break;
      case this.reclama3:
         this.reclama3._visible = false;
         _global.API.network.send("!CPO" + this.item3);
         break;
      case this.reclama4:
         this.reclama4._visible = false;
         _global.API.network.send("!CPO" + this.item4);
         break;
      case this.reclama5:
         this.reclama5._visible = false;
         _global.API.network.send("!CPO" + this.item5);
         break;
      case this.reclama6:
         this.reclama6._visible = false;
         _global.API.network.send("!CPO" + this.item6);
         break;
      case this.reclama7:
         this.reclama7._visible = false;
         _global.API.network.send("!CPO" + this.item7);
         break;
      case this.reclama8:
         this.reclama8._visible = false;
         _global.API.network.send("!CPO" + this.item8);
         break;
      case this.cerrar:
