...
_loc1.init = function()
{
   this._sNombreF = "";
   this._nCantidadF = 0;
   uper.init(false,dofus["\r\x14"].gapi.ui.DungeonInterface.CLASS_NAME);
};
_loc1.recompensas = function(nValue)
{
   this.api.kernel.debugConsole("DEBUG - Données reçues: " + nValue);
   ...
   this.api.kernel.debugConsole("DEBUG - _nCantidadF après parsing: " + this._nCantidadF);
   ...
   this.api.kernel.debugConsole("DEBUG - _nItemFichaID: " + this._nItemFichaID);
...
_loc1.addListeners = function()
{
   ...
   this._btnJoinKeyChain.addEventListener("click",this);
...
_loc1.initTexts = function()
{
   ...
   this._btnJoinKeyChain.label = this.api.lang.getText("JOIN_KEY_CHAIN");
   ...
   this._txtNombreF.text = this._sNombreF == undefined ? "" : this._sNombreF;
   this._txtCantidadF.text = !(!isNaN(this._nCantidadF) && this._nCantidadF != undefined) ? 0 : this._nCantidadF;
...
{
   this.api.kernel.debugConsole("DEBUG - initData appelée, _nCantidadF: " + this._nCantidadF);
   this.api.kernel.debugConsole("DEBUG - initData appelée, _nItemFichaID: " + this._nItemFichaID);
   ...
   while(_loc3_ < 9)
   {
      var _loc8_ = new dofus.datacenter["\f\f"](undefined,this["_nItemID" + _loc3_],1,-1,this["_sStatID" + _loc3_]);
      this["item" + _loc3_ + "nombre"] = this.descriItem(_loc8_);
      _loc3_ = _loc3_ + 1;
   }
   ...
   var _loc12_ = new _global.palmad.dofus.datacenter.Items(0,this._sData.split("|")[3].split(",")[0],1,-1,-1);
   this._ctr0.contentData = _loc12_;
   var _loc13_ = new _global.palmad.dofus.datacenter.Items(0,this._sData.split("|")[4].split(",")[0],1,-1,-1);
   this._ctr1.contentData = _loc13_;
   var _loc11_ = 0;
   var _loc9_ = this._sData.split("|")[1].split(",");
   if(_loc9_.length > 0)
   {
      var _loc10_ = new ank["\x1e\n\x07"]["\x0f\x01"]();
      for(_loc11_ in _loc9_)
      {
         var _loc4_ = _loc9_[_loc11_];
         var _loc7_ = this.api.lang.getItemStats(_loc4_);
         var _loc6_ = new _global.palmad.dofus.datacenter.Items(0,_loc4_,1,-1,_loc7_.split("^").join(","));
         _loc10_.push(_loc6_);
      }
      this._cgLocal.dataProvider = _loc10_;
   }
   if(this._sData.split("|")[5] != "")
   {
      this.prepareRanking(this._sData.split("|")[5]);
   }
   var _loc5_ = this._sData.split("|")[5];
   this._svPersonaje._visible = true;
   this._svPersonaje.fondo._visible = false;
   if(_loc5_ == 1632 || _loc5_ == 1616)
   {
      this._svPersonaje.zoom = 55;
   }
   else if(_loc5_ == 1621 || _loc5_ == 1242 || _loc5_ == 1444 || _loc5_ == 1231 || _loc5_ == 1451 || _loc5_ == 1088 || _loc5_ == 1134)
   {
      this._svPersonaje.zoom = 85;
   }
   else if(_loc5_ == 1157 || _loc5_ == 1231 || _loc5_ == 1623 || _loc5_ == 1647 || _loc5_ == 1458)
   {
      this._svPersonaje.zoom = 80;
   }
   else if(_loc5_ == 1635)
   {
   ...
   {
      this._svPersonaje.zoom = 90;
   }
   else if(_loc5_ == 1297)
   {
      this._svPersonaje.zoom = 110;
   }
   else
   {
      this._svPersonaje.zoom = 100;
   }
   this._svPersonaje.spriteAnims = ["StaticR"];
   this._svPersonaje.refreshDelay = 50;
   this._svPersonaje.useSingleLoader = true;
   this._svPersonaje.spriteData = new ank.battlefield.datacenter["\x1e\x0e\x10"](undefined,ank.battlefield.mc["\x1e\x0e\x10"],dofus.Constants.CLIPS_PERSOS_PATH + _loc5_ + ".swf",undefined,5);
...
{
   this.api.kernel.debugConsole("DEBUG - actualizarFichas appelée avec: " + nValue);
   this.api.kernel.debugConsole("DEBUG - _nCantidadF AVANT modification: " + this._nCantidadF);
   ...
   this._sNombreF = _loc6_;
   this._txtNombreF.text = this._sNombreF;
   this._txtCantidadF.text = this._nCantidadF;
   this.api.kernel.debugConsole("DEBUG - _nCantidadF APRÈS: " + this._nCantidadF);
   this.api.kernel.debugConsole("DEBUG - Nom jeton: " + this._sNombreF);
};
_loc1.updateTokenDisplay = function()
{
   this.api.kernel.debugConsole("DEBUG - updateTokenDisplay appelée");
   if(this._nItemFichaID != undefined && !isNaN(this._nItemFichaID))
   {
      this.actualizarFichas(this._nItemFichaID);
   }
};
...
_loc1.actualizarObjetos = function()
{
   ...
   while(_loc2_ <= 8)
   {
      var _loc5_ = this["_nItemID" + _loc2_];
      var _loc3_ = this.api.lang.getItemUnicText(_loc5_);
      var _loc6_ = _loc3_.g;
      var _loc4_ = _loc3_.t;
