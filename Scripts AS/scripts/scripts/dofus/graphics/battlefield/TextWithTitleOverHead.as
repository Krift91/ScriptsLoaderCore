...
_loc1.draw = function(sText, sFile, nColor, nFrame, sTitle, nColorTitle)
{
   ...
   if(this._sExperiencia != undefined)
   {
      ...
      this._txtExperiencia.textColor = 16744703;
   }
   ...
   var _loc14_ = Math.ceil(Math.max(this._sExperiencia == undefined ? 0 : this._txtExperiencia.textWidth,Math.max(this._txtText.textWidth,this._txtTitle.textWidth,this._nStarsValue <= 0 ? 0 : this._nFullWidth)) + _global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.WIDTH_SPACER * 2);
   var _loc13_ = this._height + _global.palmad.dofus.graphics.battlefield.AbstractTextOverHead.HEIGHT_SPACER;
   if(this._sTipoMob != undefined)
   {
      this.drawBackground(_loc14_,_loc13_,Number(_loc12_[2]));
   }
   else
   {
