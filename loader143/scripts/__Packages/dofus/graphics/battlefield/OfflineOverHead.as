class dofus.graphics.battlefield.OfflineOverHead extends dofus.graphics.battlefield.AbstractTextOverHead
{
   ...
   function drawClip(oSprite)
   {
      ...
      this._txtGuildName._x = this._txtSpriteName._x = (0 - _loc5_) / 2 + 30 + dofus.graphics.battlefield.AbstractTextOverHead.WIDTH_SPACER * 2;
      if(!_loc3_)
      {
         this._txtSpriteName._y = 9;
      }
      this.drawBackground(_loc5_,_loc4_,16777215);
      this.attachMovie("GAPILoader","_ldrIcon",100,{_x:Math.ceil((0 - _loc5_) / 2) + dofus.graphics.battlefield.AbstractTextOverHead.WIDTH_SPACER,_y:dofus.graphics.battlefield.AbstractTextOverHead.HEIGHT_SPACER,_height:30,_width:30,contentPath:dofus.Constants.EXTRA_PATH + oSprite.offlineType + ".swf",scaleContent:true});
   }
}
