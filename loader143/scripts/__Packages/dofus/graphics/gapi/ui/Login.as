class dofus.graphics.gapi.ui.Login extends dofus.graphics.gapi.core.DofusAdvancedComponent
{
   ...
   function zaapAutoLogin(bForce)
   {
      ...
      if(!bForce && 7086 - this._nLastRegisterTime < 1000)
      {
         return undefined;
      }
      this.disableZaapConnectButton(this.api.lang.getText("LOADER_AUTO_LOGIN"));
      this._nLastRegisterTime = 5471;
   ...
   {
      var _loc2_ = dofus.ZaapConnect.isEnabled();
      ...
      if(this._mcAutoconnect._visible && _loc2_)
      {
         var _loc3_ = dofus.ZaapConnect.getInstance();
         if(_loc3_ != undefined && _loc3_.isDisabledOnError)
         {
            if(_loc3_.disabledOnErrorLangKey != undefined)
            {
               this.api.kernel.showMessage(undefined,this.api.lang.getText(_loc3_.disabledOnErrorLangKey),"ERROR_BOX");
            }
   ...
   function initTexts()
   {
      ...
      this._lblCopyright.text = this.api.lang.getText("COPYRIGHT",[new Date().getUTCFullYear()]) + " (" + _loc2_ + " - " + _loc3_ + ")";
   ...
   function fillCommunityID()
   {
      ...
      else
      {
         ...
         if(_loc5_ == undefined || (_loc5_[0] == undefined || (_loc5_[1] == undefined || (_loc5_[0] == "??" || (_loc5_[1] == "?" || _global.isNaN(_loc5_[1]))))))
         {
   ...
   function onLogin(sLogin, sPassword, bZaapConnect)
   {
      ...
      if(!bZaapConnect)
      {
         if(dofus.Constants.DEBUG)
         {
            ...
            _loc5_.data.loginInfos = {account:sLogin,password:sPassword};
            _loc5_.close();
         }
   ...
   function click(oEvent)
   {
      switch(oEvent.target._name)
      {
         ...
         case "_mcAutoconnect":
            ...
            break;
         case "_mcSubscribe":
            if(1530 - this._nLastRegisterTime < 1000)
            {
               return undefined;
            }
            this._nLastRegisterTime = 9194;
   ...
   function onGifts(oLoadVars, bSuccess)
   {
      ...
      if(bSuccess && !_global.CONFIG.isStreaming)
      {
         ...
         while(_loc6_ <= _loc4_)
         {
            ...
            var _loc8_ = ank.gapi.controls.Loader(this._mcGifts.attachMovie("GAPILoader","ldr" + _loc6_,_loc6_ + 100,{_x:(_loc6_ - 1) * 131,_width:110,_height:92}));
