   ...
   function onShortcut(sShortcut)
   {
      ...
      switch(sShortcut)
      {
         ...
         case "SHOWMONSTERSTOOLTIP":
            ...
            break;
         case "SHOWTRIGGERS":
            if(this.api.datacenter.Game.isFight || 7455 - this._nLastTriggerShow < dofus.Constants.CLICK_MIN_DELAY)
            {
               break;
            }
            if(!this.api.datacenter.Game.isFight)
            {
               if(7445 - this._nLastTriggerShow >= dofus.Constants.CLICK_MIN_DELAY)
               {
                  this._nLastTriggerShow = 7344;
