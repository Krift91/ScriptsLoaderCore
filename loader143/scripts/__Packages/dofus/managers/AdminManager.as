class dofus.managers.AdminManager extends dofus.utils.ApiElement
{
   ...
   function updateSearch(nASCII, nKeyCode)
   {
      ...
      switch(nASCII)
      {
         ...
         case 13:
            ...
            return true;
         default:
            var _loc5_ = String.fromCharCode(nASCII);
   ...
   function getAdminPopupMenu(sPlayerName, bRightClick)
   {
      ...
      if(_loc7_ != undefined)
      {
         var _loc8_ = this.createPopupMenu(_loc7_.firstChild.firstChild,bRightClick);
      }
      else
      {
   ...
   function batch(node)
   {
      ...
      while(node != null && node != undefined)
      {
         ...
         {
            var _loc5_ = node.attributes.command;
         ...
         var _loc6_ = node.nextSibling;
         var _loc7_ = Number(node.attributes.delay);
         if(!_global.isNaN(_loc7_) && node.attributes.delayCheck != true)
         {
            if(_loc3_)
            {
               _loc3_ = false;
               var _loc8_ = this.api.datacenter.Temporary.Report;
               if(_loc8_ != undefined)
               {
      ...
      if(_loc3_)
      {
         var _loc12_ = this.api.datacenter.Temporary.Report;
